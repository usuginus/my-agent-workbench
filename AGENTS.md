...
- **作業を始める前に必ず `memory/PORTAL.md` を読む。** ワークスペース・人物・継続中の話題・学びを蒸留した長期記憶の入口。
- さらに詳しい情報が必要なら以下を参照する:
  - `memory/people/<user_id>.json` — 人物ごとのプロファイルと最近の話題
  - `memory/channels/<channel_id>.json` — チャンネルごとの情報とメンバー
  - `memory/log/YYYY-MM.jsonl` — 過去のやり取りの全ログ
- 記憶と最新の Slack 情報が矛盾する場合は Slack 側を信じる。
- 永続的に価値のある事実（人物の役割・好み、約束、学び）を得たら `memory/PORTAL.md` に反映してよい。見出し構造は維持する。
